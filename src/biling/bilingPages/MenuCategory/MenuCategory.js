...
function MenuCategoey() {
    ...
    const handleDeleteCategory = async (index) => {
        ...
        if (enteredPassword === password) {
            try {
                const preUnitName = getAllunit[index];
                const response = await axios.delete(`${BACKEND_BASE_URL}menuItemrouter/removeMenuCategory?menuCategoryId=${preUnitName.menuCategoryId}`, {
                    headers: {
                        Authorization: `Bearer ${token}`
                    }
                });
